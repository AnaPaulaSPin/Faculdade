...
public class AlocadorAssentos implements Runnable {
    ...
    public void run() {
        ...
        while (executando) {
            try {
                //Ve a fila de requisições(pessoas que querem agendar?)
                RequisicaoReserva requisicao = filaRequisicoes.poll(1, TimeUnit.SECONDS);
                //verifica sempre se tem novas requisões
                if (requisicao == null){
                     continue;
                }
    ...
    private void processarRequisicao(RequisicaoReserva requisicao) {
        try {
            if(limitadorTaxa.tryAcquire(10,TimeUnit.SECONDS)){
                ...
                if(requisicao.getCodigoAssento().equals("ANY")){
                ...
                System.out.println("[Cliente 75] RESERVE Karen Ibrahim BASIC ANY → FAIL VOO_LOTADO Voo lotado");
                System.out.println("[" + requisicao.getIdCliente() +"] RESERVE " + requisicao.getNomePassageiro() + " " + requisicao.getCategoria() + " " + requisicao.getCodigoAssento() + " → FAIL VOO_LOTADO Voo lotado");
            }

            
        } catch (Exception e) {
            e.printStackTrace();
            throw new UnsupportedOperationException("ERRO_SERVIDOR: Operação interrompida");
    ...
    private RespostaReserva alocarQualquerAssento(RequisicaoReserva requisicao) {
        try {
            List<String> assentos = mapaAssentos.getTodosCodigosAssentos();
    
            for(String ass: assentos){
                Assento assento = mapaAssentos.getAssento(ass);
                if(!assento.estaOcupado()){
                    //tratar o erro
                    if(assento.getTrava().tryLock(5, TimeUnit.SECONDS)){
                        if(!assento.estaOcupado()){
                            assento.getFlagOcupado().compareAndSet(false,true);
                            assento.ocupar(requisicao.getNomePassageiro(), requisicao.getCategoria(), gerarCodigoReserva());
                            mapaAssentos.decrementarAssentosLivres();
                            System.out.println("[" + requisicao.getIdCliente() +"] RESERVE " + requisicao.getNomePassageiro() + " " + requisicao.getCategoria() + " " + requisicao.getCodigoAssento() +" →  OK "+ assento.getCodigo() +" " + assento.getCodigoReserva()+ " VOO_LOTADO Voo lotado");
                            return RespostaReserva.sucesso(assento.getCodigo(),assento.getCodigoReserva());
                        }
        ...
        // tratar:
            ...
            e.printStackTrace();
            throw new UnsupportedOperationException("Erro ao tentar a reservar um assento tente novamente mais tarde!");
        }
