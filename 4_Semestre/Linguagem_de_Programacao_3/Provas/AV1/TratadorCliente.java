...
public class TratadorCliente implements Runnable {
    ...
    SynchronousQueue<Integer> vez = new SynchronousQueue<>();
    ...
    private String tratarReserva(String[] partes) {
        ...
        String nome = partes[1] + " " + partes[2];
        String categoria = partes[3].toLowerCase();
        String assento = partes[4];

        Categoria validar = Categoria.valueOf(categoria);

         if(validar == null){
            return "FAIL CATEGORIA_INVALIDA Categoria inválida: <categoria>";
        } 

        RequisicaoReserva reserva = new RequisicaoReserva(idCliente, nome,validar,assento);
        filaRequisicoes.offer(reserva);
        ...
        REQUISITOS:
            ...
            3. Converter categoria para UPPERCASE - txt.toLowerCase();
