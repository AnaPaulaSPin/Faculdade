...
public class ServidorLeiloeiro extends UnicastRemoteObject implements ServicoLeilao {
     ...
     * 
     * REQUISITOS:
     * 1. Usar lockLance.lock() para garantir exclusão mútua v
     * 2. Dentro do lock (try-finally): v
     *    a) Buscar leilão pelo ID, se null retornar v
     *    b) Verificar se leilão está ATIVO, se não logar e retornar v
     *    c) Chamar validarLance(leilao, compradorId, valor) v
     *    d) Se não válido, logar motivo e retornar v
     *    e) Se válido: 
     *       - Calcular liderAnterior = leilao.getCompradorLiderAtual() v
     *       - Criar Lance com UUID: new Lance(lanceId, leilaoId, compradorId, valor) v
     *       - leilao.adicionarLance(lance) v
     *       - leilao.setLanceAtual(valor) v
     *       - leilao.setCompradorLiderAtual(compradorId) v
     *       - Cancelar timeout antigo: leilao.getTarefaTimeout().cancel(false) v
     *       - Logar: "[LANCE ACEITO] Leilao=X Comprador=Y Valor=R$Z" v
     * 3. SEMPRE liberar lock no finally v
     * !!!!!!!!!!!!! TENTAR VE DPS !!!!!!!!!!!!!!
     * 4. FORA do lock:
    ...
    private void processarLance(String leilaoId, String compradorId, double valor) {
        try {
            ...
            if(leilao != null ){
                if(leilao.getStatus() == StatusLeilao.ATIVO){
                        if(validarLance(leilao, compradorId, valor)){
                            String liderAnterior = leilao.getCompradorLiderAtual();
                            ...
                            leilao.getTarefaTimeout().cancel(false);
                            log(String.format("[LANCE ACEITO] Leilao=%s Comprador=%s Valor=R$%.2f ", leilaoId, compradorId, valor));

                        } else{
                            log(String.format("LANCE INVALIDO!- Leilao=%s", leilaoId));
                            return;

                        }

                } else{
                    log(String.format("LEILAO NAO INICIADO- Leilao=%s", leilaoId));
                    return;
                }
        ...
        notificarOutbid(compradorId, leilaoId, valor, compradorId);
        agendarTimeoutInatividade(leilaoId);
    }
     ...
     * 
     * ALGORITMO:
     * 1. double lanceAtual = leilao.getLanceAtual() v
     * 2. double incrementoMinimo = lanceAtual * INCREMENTO_MINIMO_PERCENTUAL v
     * 3. double lanceMinimo = lanceAtual + incrementoMinimo v
     * 
    ...
    private boolean validarLance(Leilao leilao, String compradorId, double valor) {
        ...
        if(liderAtual != null && liderAtual.equals(compradorId)){
            return false;
        }
        if(!compradores.containsKey(compradorId)){
            return false;
    ...
    private void notificarOutbid(String compradorAnterior, String leilaoId, double novoLance, String novoLider) {
        try {
            ...
            if(comprador == null){
                log(String.format("COMPRADO ANTIGO NAO ENCONTRADO!"));
                return;
            } 
            comprador.getCallback().aoSerSuperado(leilaoId, novoLance, novoLider);
            log(String.format("NOTIFICACAO OUTBID - Comprador=%s Leilao=%s NovoLance=R$%.2f", compradorAnterior, leilaoId, novoLance));

            
        } catch (RemoteException e) {
            log(String.format("FALHA CALLBACK OUTBID - Comprador=%s desconectado", compradorAnterior));
